...
@Slf4j
@Component
public class SptWebLoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        ...
        if (SptUtil.isAjaxRequest(request)) {
            SptResponse data = new SptResponse();
            if (savedRequest == null) {
                SptUtil.makeFailureResponse(response, data.message("请通过授权码模式跳转到该页面"));
                return;
            }
            data.data(savedRequest.getRedirectUrl());
            SptUtil.makeSuccessResponse(response, data);
        } else {
            if (savedRequest == null) {

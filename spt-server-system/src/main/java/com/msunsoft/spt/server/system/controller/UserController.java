...
import com.msunsoft.spt.server.system.service.IUserDataPermissionService;
import com.msunsoft.spt.server.system.service.IUserService;
...
public class UserController {
    ...
    public SptResponse index() {
        ...
        param.setUsername(SptUtil.getCurrentUsername());
        ...
        return new SptResponse().data(data);
    }


    @GetMapping
    @PreAuthorize("hasAuthority('user:view')")
    public SptResponse userList(QueryRequest queryRequest, SystemUser user) {
        Map<String, Object> dataTable = SptUtil.getDataTable(userService.findUserDetailList(user, queryRequest));
    ...
    @GetMapping("/{userId}")
    @PreAuthorize("hasAuthority('user:update')")
    public SptResponse findUserDataPermissions(@NotBlank(message = "{required}") @PathVariable String userId) {
        String dataPermissions = this.userDataPermissionService.findByUserId(userId);
        return new SptResponse().data(dataPermissions);
    }
    ...
    public void updateProfile(@Valid SystemUser user) throws SptException {

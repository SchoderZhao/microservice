package com.msunsoft.spt.gateway.enhance.service.impl;


import com.msunsoft.spt.common.core.entity.SptResponse;
import com.msunsoft.spt.common.core.utils.DateUtil;
import com.msunsoft.spt.common.core.utils.SptUtil;
import com.msunsoft.spt.gateway.enhance.entity.*;
import com.msunsoft.spt.gateway.enhance.service.*;
import com.msunsoft.spt.gateway.enhance.utils.AddressUtil;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = SptUtil.getServerHttpRequestIpAddress(request);
                ...
                if (forbid.get()) {
                    return SptUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, new SptResponse().message("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = SptUtil.getServerHttpRequestIpAddress(request);
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return SptUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, new SptResponse().message("访问频率超限，请稍后再试"));
            } else {
